...
|---|---|---|---|---|
| 1 | **PARTIAL — Razorpay test key needs the real SECRET.** As of 2026-06-15 `server/.env.example.razorpay` has a valid-format `RAZORPAY_KEY_ID` (`rzp_test_…`) but `RAZORPAY_KEY_SECRET` is a placeholder (24× `x`) and `RAZORPAY_WEBHOOK_SECRET` is empty; the active `server/.env` is still all-blank (mock mode). Provide the **real test SECRET** (and webhook secret) | I (payment), part of B | Credentials are account-bound; secrets stay server-side | Paste the real values into `server/.env`: `RAZORPAY_KEY_ID=rzp_test_…`, `RAZORPAY_KEY_SECRET=<real>`, `RAZORPAY_WEBHOOK_SECRET=<real>` (Razorpay Dashboard → Settings → API Keys / Webhooks). Then restart `node dist/main.api`. NOTE: full checkout still also needs an Android device (action #2). |
