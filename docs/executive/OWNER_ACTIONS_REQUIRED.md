...
|---|---|---|---|---|
| 1 | **MOSTLY RESOLVED — Razorpay order creation now works live.** 2026-06-15: found the real test KEY_ID+SECRET in `radha_backend/.env.development`, copied them into the active `server/.env` → live Razorpay test order created (`order_T1pBIh3tcTEuVy`). **Still needed for the FULL flow:** (a) `RAZORPAY_WEBHOOK_SECRET` (still empty) to verify webhooks; (b) an Android device (action #2) to drive the actual checkout sheet → real payment → `/payments/verify` signature + entitlement activation | I (payment), part of B | Credentials/webhook secret are account-bound; the checkout sheet needs a device | Add `RAZORPAY_WEBHOOK_SECRET=<real>` to `server/.env` (Razorpay Dashboard → Settings → Webhooks) + provide a device |
