...
| Razorpay server-side checkout creates a REAL test order (live integration, not mock) | VERIFIED_PAYMENT | owner's test keys (from radha_backend/.env.development) copied into active server/.env; backend boot log "Razorpay provider resolved: live"; `POST /payments/checkout {planId, billingCycle:monthly}` → 200, `razorpayOrderId=order_T1pBIh3tcTEuVy`, amount 4900 paise (₹49 starter), keyId rzp_test_… (2026-06-15). **Still NOT verified:** `/payments/verify` signature + webhook (needs the Razorpay checkout sheet on a device + the webhook secret, still empty) + entitlement activation + idempotency under real callback. |
