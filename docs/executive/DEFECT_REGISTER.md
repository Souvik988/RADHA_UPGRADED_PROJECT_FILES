...
| D11 | Catalog/data | P2 | **Browse catalog thin + bundled cards not linked to real data.** Server catalog had only 2 real products (OFF rate-limit during seed); the `db:import:curated` step is NOT in the standard bring-up, so a fresh backend has an empty catalog. Mobile bundled launch-catalog cards only link to server data when `resolved_eans.g.dart` carries the EANs. Regenerating it from a richer import (9 EANs) breaks 3 catalog tests that are coupled to specific resolved-EAN fixtures (one throws a test-side StateError — confirmed NOT an app bug; `launchProductByEan` is null-safe). | PARTIALLY ADDRESSED 2026-06-15: re-ran `db:import:curated` → server catalog now **11 real products** with nutrition + health (live-verified). OPEN: (1) add catalog import to the standard seed/bring-up (OFF-dependent count); (2) regenerate `resolved_eans.g.dart` + update the 3 coupled catalog tests to be fixture-robust, then re-commit. Browse already works against the live backend (11 server products clickable; honest "scan to unlock" for unresolved). |
