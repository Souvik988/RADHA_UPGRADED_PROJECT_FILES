...
void main() {
  ...
  testWidgets('creating an expiry record posts to backend and returns to list', (
    ...
    harness.adapter.onPost('/api/v1/expiry-records', (server) {
      postReceived = true;
    ...
    expect(
      ...
      isTrue,
      reason:
          'POST /api/v1/expiry-records should fire when "Save Record" is tapped.',
    );
  });
}
...
void _stubExpiryListEmpty(DioAdapter adapter) {
  for (final status in ['yellow,red', 'expired', 'green']) {
    adapter.onGet(
      '/api/v1/expiry-records',
      (server) => server.reply(200, <Map<String, dynamic>>[]),
      queryParameters: {'limit': 20, 'status': status, 'storeId': 's-1'},
    );
  }
}
