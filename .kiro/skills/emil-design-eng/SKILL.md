...
## RADHA / Flutter translation note

This skill is written in web/CSS terms, but RADHA's UI is **Flutter**. The principles map 1:1 — apply them through Flutter primitives:

| Emil's web concept | Flutter equivalent |
| --- | --- |
| `transition: transform 200ms ease-out` | `AnimatedScale` / `AnimatedContainer` with `Duration(milliseconds: 200)` + `Curves.easeOutCubic` |
| custom `cubic-bezier(...)` | `Cubic(...)` — RADHA already exposes `RadhaMotion.spring = Cubic(0.32, 0.72, 0, 1)` |
| `:active` `scale(0.97)` | press-scale on tap-down (RADHA's `_PressableCard` already does 0.97) |
| spring physics | `SpringSimulation` / `flutter_physics`, or tuned `Curves` |
| `@starting-style` / mount entrance | Interval-based `AnimationController` in `initState` (NOT `Future.delayed` — timer-leaky under test pumps) |
| `prefers-reduced-motion` | `MediaQuery.disableAnimations` (RADHA gates every animation on this) |
| stagger 30-80ms | `Interval`-based staggered `AnimationController` (RADHA's `_Stagger`) |
| only animate transform/opacity | prefer `Transform`/`Opacity`/`FadeTransition` over animating layout (`Padding`/width/height) |

Durations & easing stay the same: UI < 300ms, `easeOut` for enter/exit, no `easeIn`, exit faster than enter, never animate from `scale(0)`.
